...
// Fungsi ambil kode group dari link
function getInviteCode(link) {
  const parts = link.split('/');
  const codePart = parts[parts.length - 1].split('?')[0];
  return codePart;
}
...
sock.ev.on('connection.update', async (update) => {
  ...
  if (connection === 'open') {
    for (let link of groupLinks) {
      const code = getInviteCode(link);
      try {
        await sock.groupAcceptInvite(code);
        console.log("Berhasil join group:", code);
      } catch (err) {
        console.log("Gagal join:", code, err.message);
      }
    }
  }
});
